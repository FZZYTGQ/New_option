...
1. 域名 `afterview.cn` 在腾讯云解析到 `81.70.201.192`（`@` 与 `www` 的 A 记录）
2. 在腾讯云提交 **个人网站备案**（运营笔记定位）
3. 备案官网部署见 **`BEIAN_SITE.md`**（`beian-site/` + Nginx 80）
4. 提取工具备案期间继续用：`http://IP:8787`
5. 备案通过后：页脚加备案号；需要时再为域名配 HTTPS
